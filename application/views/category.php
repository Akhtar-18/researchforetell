...
<body>
                    ...
                    <div class="category_class">
                        ...
                        <ul class="category_page_ul">
                            <?php $cats = $this->custom->get_all_parent_category();  foreach($cats as $c) {   ?> <li class="category_page_li">
                                <a href="<?php echo base_url('category/'.$c['id'].'/'.$c['categorySlug']);?>"><?= $c['categoryName'];?></a>(<?php echo $this->custom->categoryReportCount($c['id']);?>)</li>
                            <?php } ?>
    ...
    font-size: 26px;
    margin: 0 0 10px;
                                                  padding: 0 0 10px;font-size:20px;"><?= strtoupper($categoryName);?> MARKET RESEARCH REPORTS <a href="<?=base_url();?>contact" target="_blank"><button class="btn btn-default brn-sm float-right">FOLLOW</button></a></h3>
             ...
             <?php if($all_count > 0) { ?>
                        <div class="col-md-12 col-sm-12 col-lg-12 col-12">
                            <div class="mt-10" style="margin: 20px;">
                                <center><button class="btn btn-sm btn-primary" id="loadMore">Load More</button></center>
                            </div>
                        </div>
                        <?php } ?>
